...
Scope {
    ...
    property string captureType: "screenshot"  // "screenshot", "recording"
    property string mode: "region"  // "region", "window", "screen"
    property bool editMode: false
    property bool suppressSelectionAnimations: false
    ...
    property real selectedWindowGlobalX: 0
    property real selectedWindowGlobalY: 0
    property real selectedWindowGlobalWidth: 0
    property real selectedWindowGlobalHeight: 0
    ...
    readonly property var captureTypes: ["screenshot", "recording"]
    readonly property var modeIcons: ({
            region: "󰩭",
            window: "󰖯",
            screen: "󰍹"
        })
    readonly property var captureTypeIcons: ({
            screenshot: "󰄀",
            recording: "󰑋"
        })
    ...
    Behavior on selectionX {
        enabled: Config.screenshotAnimations && !root.suppressSelectionAnimations
        SpringAnimation {
            spring: 4
            damping: 0.4
        }
    ...
    function startCapture(type: string) {
        root.captureType = type || "screenshot";
        prepareCapture();
    ...
    function resetSelection() {
        ...
        root.selectedWindowGlobalX = 0;
        root.selectedWindowGlobalY = 0;
        root.selectedWindowGlobalWidth = 0;
        root.selectedWindowGlobalHeight = 0;
    }
    ...
    function setCaptureType(newType: string) {
        const currentMode = root.mode;
        resetSelection();
        root.captureType = newType;
    ...
    function confirmSelection() {
        root.editMode = false;
        if (root.captureType === "recording") {
            root.startRecording(root.selectionX, root.selectionY, root.selectionWidth, root.selectionHeight);
            return;
        }
    ...
    function checkWindowAt(mouseX: real, mouseY: real, screenName: string) {
        ...
        for (let i = windows.length - 1; i >= 0; i--) {
            ...
            if (mouseX >= winX && mouseX <= winX + winW && mouseY >= winY && mouseY <= winY + winH) {
                ...
                root.selectedWindowGlobalX = win.at[0];
                root.selectedWindowGlobalY = win.at[1];
                root.selectedWindowGlobalWidth = winW;
                root.selectedWindowGlobalHeight = winH;
                return;
            }
        }
        resetSelection();
    ...
    function prepareCapture() {
        root.captureType = root.captureType || "screenshot";
        root.mode = "region";
        resetSelection();
    ...
    function selectedMonitorIpc() {
        const monitorName = root.hyprlandMonitor?.name || root.activeScreen?.name || "";
        return root.monitorsFromIpc.find(m => m.name === monitorName);
    }
    ...
    function startRecording(x: real, y: real, width: real, height: real) {
        if (width < 5 || height < 5)
            return;

        const monitor = root.selectedMonitorIpc();
        const monitorName = root.hyprlandMonitor?.name || root.activeScreen?.name || "";
        ...
        if (root.mode === "screen" && monitorName !== "") {
            RecordingService.startOutput(monitorName);
            return;
        }

        if (root.mode === "window" && root.selectedWindowGlobalWidth > 0 && root.selectedWindowGlobalHeight > 0) {
            const windowGeometry = Math.round(root.selectedWindowGlobalX) + "," + Math.round(root.selectedWindowGlobalY) + " " + Math.round(root.selectedWindowGlobalWidth) + "x" + Math.round(root.selectedWindowGlobalHeight);
            RecordingService.startGeometry(windowGeometry);
            return;
        }
        ...
        const globalX = Math.round(monitor.x + x);
        const globalY = Math.round(monitor.y + y);
        const geometry = globalX + "," + globalY + " " + Math.round(width) + "x" + Math.round(height);
        RecordingService.startGeometry(geometry);
    }
